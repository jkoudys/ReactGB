...
/**
 * Attach a hq4 renderer from a hidden source context, to a visible one
 *
 * @param canvas source
 * @param canvas target
 */
...
var _target;
var _targetCount = 0;
export function hqAttach(source, target) {
  _source = source;
  _target = target;
  _sourceCount = _source.width * _source.height;
  _targetCount = _sourceCount * 4;
}

/**
 * Refresh the attached context
 *
 */
export function hqRefresh() {
  // Hard-set at 4x scale, as CSS engine will scale beyond that
}

function hq(img, scale) {
...
export default function hq4x(srcImage) {
  ...
  // Destination array is 16x the size of source
  var dest = _dest = new Uint8ClampedArray(width * height << 4);
